...
export async function channelHtml(request, response) {
  ...
  // Render each item card using the nunjucks singleton (already configured
  // with plugin view paths by @indiekit/frontend at startup)
  const renderLocals = {
    channel: channelDocument,
    baseUrl: request.baseUrl,
    __: request.__,
  ...
  const html = timeline.items
    .map((item) => nunjucks.render("partials/item-card.njk", { ...renderLocals, item }))
    .join("");
...
export async function timelineHtml(request, response) {
  ...
  const html = result.items
    .map((item) => {
      const cardHtml = nunjucks.render("partials/item-card.njk", { ...renderLocals, item });
      const badge = item._channelName
        ...
        : "";
      return `<div class="ms-timeline-view__item">${badge}${cardHtml}</div>`;
    })
    .join("");
