...
export function startScheduler(indiekit) {
  ...
  // Defer background sync if INDIEKIT_DEFER_BACKGROUND_SYNC is set
  // (start.sh sets this during initial boot + Eleventy build)
  if (process.env.INDIEKIT_DEFER_BACKGROUND_SYNC === "1") {
    console.log("[Microsub] Feed polling deferred — waiting for /tmp/indiekit-ready-for-sync");
    const deferCheck = setInterval(async () => {
      try {
        const fs = await import("node:fs");
        if (fs.existsSync("/tmp/indiekit-ready-for-sync")) {
          clearInterval(deferCheck);
          console.log("[Microsub] Sync marker found — starting feed polling");
          actuallyStartScheduler();
        }
      } catch {
        // File not ready yet
      }
  ...
  function actuallyStartScheduler() {
    // Run every minute
